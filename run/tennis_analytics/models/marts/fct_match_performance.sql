        ...
        create or replace table `tennis_dev`.`silver_gold`.`fct_match_performance`
    ...
    using delta
      ...
      as
      with matches as (
    select * from `tennis_dev`.`silver_int`.`int_match_with_opponent_strength`
),
...
final as (
    select
        -- Surrogate key
        md5(cast(concat(coalesce(cast(match_id as string), '_dbt_utils_surrogate_key_null_')) as string))
                                                        as match_sk,
        m.match_id,
        p.player_sk,
        m.player_id,
