  ...
  create or replace view `tennis_dev`.`silver_stg`.`stg_training_sessions`
  
  as (
    with source as (
    select * from `tennis_dev`.`bronze`.`raw_training_sessions`
),

name_map as (
    select * from `tennis_dev`.`silver_stg`.`stg_player_name_mapping`
),
